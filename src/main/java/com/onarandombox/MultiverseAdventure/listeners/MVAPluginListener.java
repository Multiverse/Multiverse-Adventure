    ...
    public void onPluginEnable(PluginEnableEvent event) {
        if (event.getPlugin().getDescription().getName().equals("Multiverse-Core")) {
            MultiverseAdventure.getInstance().setCore(
                    ((MultiverseCore) MultiverseAdventure.getInstance().getServer().getPluginManager().getPlugin("Multiverse-Core")));
            ...
            if (event.getPlugin().isEnabled()) {
                ...
                MultiverseAdventure.staticLog(Level.WARNING,
                        "I just disabled the old version of Multiverse for you. You should remove the JAR now, your configs have been migrated.");
            }
        }
    }
